...
// -------------------- EXPRESS (puerto falso para Render) --------------------
const app = express();
const PORT = process.env.PORT || 3000;

app.get("/", (req, res) => {
  res.send("🤖 Bot de Telegram corriendo en Render Free 🚀");
});
...
const getInventario = async () =>
  (await fetch(`${BACKEND_URL}/api/inventario`)).json();
...
bot.onText(/\/stock/, async (msg) => {
  ...
  try {
    ...
    if (!inventario.length)
      return bot.sendMessage(chatId, "📦 Inventario vacío.");
    const texto = inventario
      .map((i) => `- ${i.producto}: ${i.cantidad}`)
      .join("\n");
...
bot.onText(/\/agregar (.+) (\d+) (\S+)/, async (msg, match) => {
  ...
  if (!producto || cantidad <= 0) {
    return bot.sendMessage(
      chatId,
      "❌ Formato: /agregar Broca 10 miClaveSecreta123"
    );
  }
...
bot.onText(/\/pedidos/, async (msg) => {
  ...
  try {
    ...
    const texto = pedidos
      .map((p) => `#${p.id} ${p.producto} x${p.cantidad} (${p.estado})`)
      .join("\n");
...
bot.on("message", async (msg) => {
  ...
  partes.forEach((p) => {
    ...
    if (key && value)
      pedido[key.trim()] = isNaN(value.trim())
        ? value.trim()
        : Number(value.trim());
  });
  ...
  if (
    ...
    return bot.sendMessage(
      chatId,
      "❌ Formato inválido: cliente=Juan, producto=Broca, cantidad=5, proveedor=XYZ"
    );

  try {
    const inventario = await getInventario();
    const item = inventario.find(
      (i) => i.producto.toLowerCase() === pedido.producto.toLowerCase()
    );
    if (!item || item.cantidad < pedido.cantidad)
      return bot.sendMessage(
        chatId,
        `❌ Stock insuficiente de ${pedido.producto}. Disponible: ${
